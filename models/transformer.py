...
from diffusers.models.attention import GatedSelfAttentionDense, AdaLayerNorm, AdaLayerNormZero, FeedForward
...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        x_local = rearrange(hidden_states, 'b (h w) c -> b c h w', h=height)
        x_local = self.local1(x_local)
        x_local = rearrange(x_local, 'b c h w -> b (h w) c')

        # 0. Self-Attention
        if self.use_ada_layer_norm:
            norm_hidden_states = self.norm1(x_local, timestep)
        elif self.use_ada_layer_norm_zero:
            norm_hidden_states, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.norm1(
                x_local, timestep, class_labels, hidden_dtype=x_local.dtype
            )
        else:
            norm_hidden_states = self.norm1(x_local)
