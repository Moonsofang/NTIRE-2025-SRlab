...
class RAMLora(nn.Module):
    ...
    def generate_tag(self,
        ...
        #     return flops.total()
        # class WrappedTaggingHead(nn.Module):
        #     def __init__(self, tagging_head, image_embeds, image_atts):
        #         super().__init__()
        #         self.tagging_head = tagging_head
        #         self.image_embeds = image_embeds
        #         self.image_atts = image_atts

        #     def forward(self, label_embed):
        #         return self.tagging_head(
        #             encoder_embeds=label_embed,
        #             encoder_hidden_states=self.image_embeds,
        #             encoder_attention_mask=self.image_atts,
        #             return_dict=False,
        #             mode='tagging'
        #         )[0]  # 只返回需要计算的 embedding
        
        # flops1 = calculate_flops(self.visual_encoder, image)
        # image_n = self.visual_encoder(image)
        # flops2 = calculate_flops(self.image_proj, image_n)
        ...
        # wrapped_tagging_head = WrappedTaggingHead(self.tagging_head, image_embeds, image_atts)
        # flops3 = calculate_flops(wrapped_tagging_head, label_embed)

        logits = self.fc(tagging_embed[0]).squeeze(-1)
        # flops4 = calculate_flops(self.fc, tagging_embed[0])
                
        # print(f"总 FLOPs: {flops1+flops2+flops3+flops4}")
        # exit()
