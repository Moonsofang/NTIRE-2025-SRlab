...
from models.controlnet_local import ControlNetModel
from models.unet_2d_condition_local import UNet2DConditionModel
...
if args.controlnet_model_name_or_path:
    ...
    controlnet = ControlNetModel.from_pretrained(args.controlnet_model_name_or_path, subfolder="controlnet",ignore_mismatched_sizes=True,low_cpu_mem_usage=False)
...
for name, module in unet.named_modules():
    if any(trainable_module in name for trainable_module in args.trainable_modules):
...
RAM = ram(pretrained='/media/ssd8T/ly/SeeSR/preset/models/ram_swin_large_14m.pth',
...
# with open("unet_trainable.txt", "w") as f:
#     for param in param_list:
#         f.write(param + "\n")
print('>>>>>>>>>>>>>>>>controlnet trainable>>>>>>>>>>>>>>>>>>>>')
param_list = []
for name, params in controlnet.named_parameters():
    if params.requires_grad:
        param_list.append(name)
print(len(param_list))
# with open("controlnet_trainable.txt", "w") as f:
#     for param in param_list:
#         f.write(param + "\n")
# Optimizer creation
