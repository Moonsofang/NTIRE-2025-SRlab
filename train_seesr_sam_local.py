...
from dataloaders.paired_sam_dataset import SAMPairedCaptionDataset
...
if args.unet_model_name_or_path:
    ...
    print(f'===== if use sam encoder? {unet.config.use_image_cross_attention}')
...
train_dataset = SAMPairedCaptionDataset(root_folders=args.root_folders,
                                    tokenizer=tokenizer,
...
sam2.to(accelerator.device, dtype=weight_dtype)
...
for epoch in range(first_epoch, args.num_train_epochs):
    for step, batch in enumerate(train_dataloader):
        ...
        with accelerator.accumulate(controlnet), accelerator.accumulate(unet):
            ...
            with torch.no_grad():
                sam_image = batch["sam_values"].to(accelerator.device, dtype=weight_dtype)
                sam_encoder_hidden_states = SAM.generate_image_embedding(sam_image)# b,4096,256
            ...
            down_block_res_samples, mid_block_res_sample = controlnet(
                ...
                return_dict=False,
                image_encoder_hidden_states=sam_encoder_hidden_states,
            )
