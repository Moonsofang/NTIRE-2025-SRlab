...
def log_validation(transformer, vae, text_encoder, tokenizer, text_encoder_2, tokenizer_2, text_encoder_3, tokenizer_3, controlnet, args, accelerator, weight_dtype, step):
    ...
    for validation_prompt, validation_image in zip(validation_prompts, validation_images):
        ...
        for _ in range(args.num_validation_images):
            with torch.autocast("cuda"):
                image = pipeline(
                    prompt = validation_prompt, control_image = validation_image, num_inference_steps=28, generator=generator
                ).images[0]
...
# )
text_encoder = CLIPTextModelWithProjection.from_pretrained(args.pretrained_model_name_or_path, subfolder = 'text_encoder', revision=args.revision)
text_encoder_2 = CLIPTextModelWithProjection.from_pretrained(args.pretrained_model_name_or_path, subfolder = 'text_encoder_2', revision=args.revision)
text_encoder_3 = T5EncoderModel.from_pretrained(args.pretrained_model_name_or_path, subfolder = 'text_encoder_3', revision=args.revision)
...
if args.transformer_model_name_or_path:
    ...
    unet = SD3Transformer2DModel.from_pretrained_orig(
        args.pretrained_model_name_or_path, args.transformer_model_name_or_path, subfolder="transformer", revision=args.revision, use_image_cross_attention=True
    )
else:
    # resume from pretrained SD
    logger.info("Loading unet weights from SD")
    unet = SD3Transformer2DModel.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="transformer", revision=args.revision, use_image_cross_attention=True
    )
...
if args.controlnet_model_name_or_path:
    ...
    controlnet = SD3ControlNetModel.from_transformer(transformer, use_image_cross_attention=True)
...
if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
    ...
    def save_model_hook(models, weights, output_dir):
        ...
        for i, model in enumerate(models):
            sub_dir = "transformer" if isinstance(model, SD3Transformer2DModel) else "controlnet"
            model.save_pretrained(os.path.join(output_dir, sub_dir))
    ...
    def load_model_hook(models, input_dir):
        ...
        for i in range(len(models)):
            ...
            # load diffusers style into model
            if not isinstance(model, SD3Transformer2DModel):
                load_model = SD3ControlNetModel.from_pretrained(input_dir, subfolder="controlnet") # , low_cpu_mem_usage=False, ignore_mismatched_sizes=True
            else:
...
params_to_optimize = list(controlnet.parameters()) + list(transformer.parameters())
...
for epoch in range(first_epoch, args.num_train_epochs):
    for step, batch in enumerate(train_dataloader):
        ...
        with accelerator.accumulate(controlnet), accelerator.accumulate(transformer):
            ...
            prompt_embed = text_encoder(batch["input_ids"].to(accelerator.device))
            pooled_prompt_embed = prompt_embed[0]
            prompt_embed = prompt_embed.hidden_states[-2]
            
            prompt_2_embed = text_encoder_2(batch["input_ids_2"].to(accelerator.device))
            pooled_prompt_2_embed = prompt_2_embed[0]
            prompt_2_embed = prompt_2_embed.hidden_states[-2]
            
            clip_prompt_embeds = torch.cat([prompt_embed, prompt_2_embed], dim=-1)
            
            t5_prompt_embed = text_encoder_3(batch["input_ids_3"].to(accelerator.device))[0]
            
            clip_prompt_embeds = torch.nn.functional.pad(
                clip_prompt_embeds, (0, t5_prompt_embed.shape[-1] - clip_prompt_embeds.shape[-1])
            )
            encoder_hidden_states = torch.cat([clip_prompt_embeds, t5_prompt_embed], dim=-2)
            ...
            if accelerator.sync_gradients:
                ...
                params_to_clip = list(controlnet.parameters()) + list(transformer.parameters())
